...
if __name__ == '__main__':
    ...
    # load source
    cap = cv2.VideoCapture("http://10.10.10.116:81/stream")
    ...
    while True:
        if cap.isOpened():
            ret, frame = cap.read()
            value1 = cv2.getTrackbarPos('Value 1', 'frame2')
            value2 = cv2.getTrackbarPos('Value 2', 'frame2')
            if ret:
            #     cropped = frame[y_l:y_h,x_l:x_h]
                cropped_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                _, threshold = cv2.threshold(cropped_gray, value1, 255, 0 )
                output = threshold
            ...
            for contour in contours:
                if (500 > cv2.contourArea(contour) > 150):
                    boundingRect = cv2.boundingRect(contour);
            #         if (boundingRect[3] > 35):                          # filter contours based on height
                    cv2.rectangle(frame,boundingRect,(255,255,0),1) # display
